...
30. **M3-21 Incident Workspace Keyboard Navigation / Accessibility QA UX 收口**已交付（2026-06-22）：在 M3-17 到 M3-20 案件工作台能力之上补齐键盘导航、焦点状态和 ARIA 语义；`IncidentSection.tsx` 为工作台和左右区域补 region 名称，`IncidentList.tsx` 将多选 checkbox accessible name 改为只含 incident id、状态、严重度和告警数，并用 `aria-current` / `aria-selected` 标识当前详情案件；`IncidentDetailPanel.tsx` 为 status / severity radiogroup 增加方向键、Home/End 导航，并为报告预览实现打开后 focus 进入 preview、Escape/关闭后恢复到 `incident-preview-report`；Evidence Pack / Closure Review checklist 和导出队列补 list/listitem 语义与 focus-visible。新增 `server/tests/test_incident_workspace_accessibility_e2e.py`，真实浏览器覆盖键盘筛选、Space 多选、Tab 到列表项、Enter 打开详情、radiogroup 方向键、保存、报告预览焦点恢复、Evidence/Closure 刷新复制、批量复制/队列清空、accessible name audit、重复 id audit、storage audit、DOM/clipboard forbidden sentinel 和桌面/移动截图。截图证据：`docs/runs/artifacts/m3-21-incident-workspace-accessibility/accessibility-desktop.png` / `accessibility-mobile.png`。**真实验证**：新增 keyboard/a11y E2E `1 passed in 11.53s`；相邻案件 UX 回归首轮 `3 passed / 4 setup failures` 后 fresh backend 复跑失败项 `4 passed in 34.32s`；关键 E2E 串跑 `14 passed / 2 setup failures` 后补 stable 账号复跑 timeline 与 runbook，最终 16 个真实浏览器测试均有通过证据；后端 incident/report 契约 `31 passed in 7.06s`；后端全量 `344 passed, 17 skipped, 17 warnings`；Guardrails `139 passed, 17 warnings`；前端 `npm run typecheck` 与 `npm run build` 通过（`/dashboard` 57.8 kB / First Load JS 205 kB）。**未改**认证/授权、Guardrails、SSRF、DB schema、后端 incident/report API、npm 依赖或 rate limit；未新增后端能力；未自动关闭/删除/批量修改案件；未调用 LLM；未使用 `localStorage` / `sessionStorage` / `dangerouslySetInnerHTML` / `innerHTML`；未提交 `.coverage` / 真实 env / 数据库 / 密钥。运行日志：`docs/runs/2026-06-22-m3-21-incident-workspace-keyboard-navigation-accessibility-qa-ux.md`。

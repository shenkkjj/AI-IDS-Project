...
```text
暂无已固化的下一条启动口令；请 owner 先从下方候选中选择一个方向，再写入 `docs/agent/*.md` L4/L5 超长任务文档后启动。
```

### 下一条建议工单（M3-13 已交付后）

M3-13 已完成 Dashboard 移动 viewport 视觉 QA 收口。下一条建议先在下方候选中二选一：如果目标是继续打磨 demo 可见体验，优先 Incident report Markdown payload preview UX；如果目标是处理 M3-12 发现的长时运行稳定性债务，则单独授权 Guardrails moderation httpx pool 健康监控工单。

后续候选保留：

1. **Incident report Markdown payload preview UX 收口**：M3-07 报告导出已交付，但 markdown 中 payload preview 在 dashboard 复制按钮 / 下载按钮的 UX 还可补齐 toast / loading state 细节。

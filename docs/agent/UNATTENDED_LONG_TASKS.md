...
```text
请先固化 `docs/agent/M3_18_INCIDENT_CLOSURE_POST_INCIDENT_REVIEW_CHECKLIST_UX_TASK.md`，然后执行其中定义的 L5 超长任务。建议目标：在案件状态、证据包和报告能力之上，新增 Incident closure / post-incident review checklist UX，帮助 owner 判断案件是否可进入 `resolved / false_positive`。只允许前端 closure checklist UX、E2E、截图和文档同步；不要修改认证/授权/Guardrails/SSRF/DB schema/后端 incident/report API/npm 依赖/rate limit，不调用 LLM，不使用 localStorage/sessionStorage 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。通过新增 closure checklist E2E、既有 evidence pack / incident report / preview / security timeline / Dashboard responsive E2E、关键 E2E 串跑、后端全量、Guardrails、前端 typecheck/build 后，精确拆分 commit 并 push 到 `origin/main`，完成后输出最终报告。
```

### 下一条建议工单（M3-17 已交付后）

M3-17 已交付为 Incident / Alert evidence pack checklist UX 收口任务。下一条默认建议继续走产品体验路线：**M3-18 Incident closure / post-incident review checklist UX**，先固化任务文档，再在案件状态、证据包和报告能力之上补一个关闭前检查清单，帮助 owner 判断案件是否可以进入 `resolved / false_positive`。

后续候选保留：

1. **Incident closure / post-incident review checklist UX**：在案件状态、证据包和报告能力之上，补一个关闭前检查清单，帮助 owner 判断案件是否可以进入 `resolved / false_positive`。

...
```text
请创建 `docs/agent/M3_16_DASHBOARD_OPERATIONAL_RUNBOOK_HEALTH_CHECKLIST_UX_TASK.md` 并执行其中定义的 L5 超长任务。先完整阅读 `AGENTS.md`、`CLAUDE.md`、`PRODUCT.md`、`docs/agent/UNATTENDED_LONG_TASKS.md`、M3-15 运行日志和相关 Dashboard 组件，只允许前端 Dashboard operational runbook / health checklist UX、真实浏览器 E2E、截图和文档同步。目标是在不修改认证/授权/Guardrails/SSRF/DB schema/后端 API/npm 依赖/rate limit 的前提下，把 `/health` 可达性、env security check 提示、demo readiness 和 E2E readiness 汇总成一个可截图、可复制诊断摘要、可在移动端阅读的运维检查面板。不要使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。通过新增 runbook/health checklist E2E、Dashboard responsive E2E、九组关键 E2E、后端全量、Guardrails、前端 typecheck/build 后，精确拆分 commit 并 push 到 `origin/main`，完成后输出最终报告。
```

### 下一条建议工单（M3-15 已交付后）

M3-15 已完成 SOC timeline drilldown / filter UX 收口。下一条建议固化为 **M3-16 Dashboard operational runbook / health checklist UX 收口**，把 `/health`、env security check、demo readiness 和 E2E readiness 汇总成 owner 可自助排障的运维检查面板，并继续保持真实浏览器截图和 forbidden sentinel 验收。

后续候选保留：

1. **Incident / alert evidence pack 下载清单 UX**：在现有报告与时间线能力之上，补一个只读证据包清单视图，列出可下载报告、关联告警、时间线事件和脱敏状态，不新增后端导出格式。

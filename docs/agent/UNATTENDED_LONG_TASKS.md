...
```text
暂无已固化的下一条启动口令；默认建议先创建 `docs/agent/M3_15_SOC_TIMELINE_DRILLDOWN_FILTER_UX_TASK.md`，聚焦 SOC timeline drilldown / filter UX 收口。
```

### 下一条建议工单（M3-14 已交付后）

M3-14 已完成案件报告预览 UX 收口。下一条默认推荐继续产品体验方向：**M3-15 SOC timeline drilldown / filter UX 收口**，为审计时间线补筛选、详情展开、脱敏 sentinel E2E 和截图证据。

后续候选保留：

1. **SOC timeline drilldown / filter UX 收口**：为 `GET /logs/security-timeline` 的 Dashboard 展示补筛选、详情展开、脱敏 sentinel E2E 和截图证据，继续提升 demo 可解释性。

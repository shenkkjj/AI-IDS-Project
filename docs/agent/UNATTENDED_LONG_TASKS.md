...
```text
请先固化 `docs/agent/M3_19_CLOSED_INCIDENT_ARCHIVE_STATUS_FILTER_UX_TASK.md`，然后执行 M3-19 Closed Incident Archive / Status Filter UX L5 超长任务：在案件列表中补状态筛选、关闭态归档视图和 `closed_at` 可见性。要求先完整阅读必读上下文、创建运行日志、RED→GREEN→IMPROVE、新增真实浏览器 E2E、截图、文档同步、验证矩阵、精确 commit 和 push；只允许前端案件列表筛选/关闭态归档 UX、E2E、截图和文档同步，不新增 DB schema，不改后端权限模型、认证/授权、Guardrails、SSRF、后端 incident/report API、npm 依赖或 rate limit，不自动关闭案件，不调用 LLM，不使用 localStorage/sessionStorage 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。完成后输出最终报告。
```

### 下一条建议工单（M3-18 已交付后）

M3-18 已交付。默认建议继续走产品体验路线：**M3-19 Closed Incident Archive / Status Filter UX**，在案件列表中补状态筛选、关闭态归档视图和 `closed_at` 可见性，基于现有 incidents 查询能力和状态字段，不新增 DB schema，不改后端权限模型。

后续候选保留：

1. **Closed Incident Archive / Status Filter UX**：在案件列表中补状态筛选、关闭态归档视图和 `closed_at` 可见性，基于现有 incidents 查询能力和状态字段，不新增 DB schema，不改后端权限模型。

...
```text
请先创建 `docs/agent/M3_11_DASHBOARD_SECTION_RESPONSIVE_QA_TASK.md`，再执行其中定义的 L5 超长任务。目标是在 M3-10 已完成 route composition 拆分后，只做浏览器级响应式 QA 与可访问性收口：完整阅读 `PRODUCT.md`、`AGENTS.md`、`CLAUDE.md`、`docs/agent/UNATTENDED_LONG_TASKS.md`、`docs/runs/2026-06-19-m3-10-dashboard-route-composition.md` 和 Dashboard section 组件；创建 `docs/runs/2026-06-19-m3-11-dashboard-section-responsive-qa.md` 运行日志；用 Playwright 覆盖桌面和移动视口下六个 Dashboard route tab、核心 section wrapper、顶部/移动导航 `incidents` 入口、按钮文字不溢出、section 不重叠、DOM forbidden sentinel 不出现。不要重做视觉设计，不要迁移状态管理，不要新增后端 API / npm 依赖，不要修改认证/授权/Guardrails/SSRF/DB schema，不要把 token 写进 storage / DOM，不要使用 git add .，不要提交 `.coverage`、真实 env、数据库或密钥。满足新增响应式 E2E、Auth/Demo/Incident/Dashboard 关键 E2E、后端全量、Guardrails、前端 typecheck/build 后，按精确 staged set commit 并 push 到 `origin/main`。
```

### 下一条建议工单（M3-10 已交付后）

**L5 / M3-11：Dashboard section 响应式 QA 与可访问性收口**。M3-10 已把 Dashboard route JSX 拆到 section 组件、统一 route metadata，并用 E2E 锁住六个 route tab 与核心 wrapper；下一步最小风险工单不是继续加业务功能，而是在桌面/移动真实浏览器里验证这些 section 的布局、导航、文字溢出、焦点/aria 状态和 forbidden sentinel，补齐响应式 E2E 与截图证据。建议先固化为 `docs/agent/M3_11_DASHBOARD_SECTION_RESPONSIVE_QA_TASK.md`，范围限定在测试、运行日志、必要的轻量 UI 可访问性修复和文档同步，继续禁止改 auth/Guardrails/SSRF/DB schema。

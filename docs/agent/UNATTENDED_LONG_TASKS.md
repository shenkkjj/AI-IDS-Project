...
```text
请基于 M3-12 的成果挑选下一条最小风险工单（建议候选：M3-13 Dashboard 移动 viewport 视觉精修 —— 复用 M3-11 截图证据排查移动 overview/incidents 页面字号/间距/卡片层级；或 M3-13 Incident report Markdown payload preview UX 收口 —— 补 toast / loading state；或 M3-13 Guardrails moderation httpx pool 健康监控 —— M3-12 排障发现长时运行 backend 中 OpenAIModerationClient 偶发退化导致 < 1.5s rail timeout 内 fail-closed 拦截 Copilot，此条需 owner 单独授权动 Guardrails）。在写新任务文档前必须读 `PRODUCT.md`、`AGENTS.md`、`CLAUDE.md`、`docs/agent/UNATTENDED_LONG_TASKS.md` 与 `docs/runs/2026-06-19-m3-12-demo-flow-e2e-stability.md`，把候选包装成 L5 阶段化长任务（运行日志、TDD 红绿、验证矩阵、停止条件、精确 commit / push 边界），落到 `docs/agent/*.md`，再发短启动口令。继续禁止改认证/授权/Guardrails/SSRF/DB schema/rate limit 常量，禁止 `git add .`，禁止提交 `.coverage` / 真实 env / 数据库 / 密钥。
```

### 下一条建议工单（M3-12 已交付后）

**L5 / M3-13 候选**：
1. **Dashboard 移动 viewport 视觉精修**：基于 `docs/runs/artifacts/m3-11-dashboard-responsive/mobile-*.png` 排查移动端字号、卡片层级、间距是否需要轻量修复，仍限定 layout class 调整，不重做视觉。
2. **Incident report Markdown payload preview UX 收口**：M3-07 报告导出已交付，但 markdown 中 payload preview 在 dashboard 复制按钮 / 下载按钮的 UX 还可补齐 toast / loading state 细节。
3. **Guardrails moderation httpx pool 健康监控（需 owner 单独授权）**：M3-12 根因排查暴露长时运行 backend 中 `OpenAIModerationClient._shared_client` 偶发退化导致 moderation 在 < 1.5s rail timeout 内立即抛 exc → fail-closed 拦截 Copilot；fresh backend 行为正确。下一条工单可补 client 健康探测 + 周期性重建 + 网络不可达时的 graceful degrade。**该工单会触碰 `server/security/llm_guardrails/**`，必须 owner 单独授权**。
任何候选都必须先固化成 `docs/agent/M3_13_*_TASK.md` 工单文档，再发短启动口令，禁止改 auth / Guardrails / SSRF / DB schema / rate limit 常量。

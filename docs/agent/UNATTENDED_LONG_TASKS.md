...
```text
请执行下一条 L5 超长任务：新增 Incident Workspace Keyboard Navigation / Accessibility QA UX。先创建任务文档和运行日志，按 RED→GREEN→IMPROVE 新增真实浏览器 E2E，覆盖案件列表、状态筛选、多选、详情面板、Evidence/Closure checklist 的键盘导航、焦点状态、ARIA 标签、桌面/移动截图和 forbidden sentinel；只允许前端可访问性/键盘导航 UX、E2E、截图和文档同步，不改认证/授权、Guardrails、SSRF、DB schema、后端 incident/report API、npm 依赖或 rate limit，不调用 LLM，不使用 localStorage/sessionStorage 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。完成完整验证矩阵后精确拆分 commit 并 push 到 `origin/main`，最后输出最终报告。
```

### 下一条建议工单（M3-20 已交付后）

M3-20 Incident Workbench Bulk Selection / Export Queue UX 已交付。默认建议继续走产品体验路线：**M3-21 Incident Workspace Keyboard Navigation / Accessibility QA UX**，在不改变认证/授权、后端 API 和数据模型的前提下，为案件列表、详情、筛选、多选和检查清单补键盘导航、焦点状态、ARIA 标签、可访问性 E2E 与截图证据。

后续候选保留：

1. **Incident Workspace Keyboard Navigation / Accessibility QA UX**：为案件列表、详情、筛选、多选和检查清单补键盘导航、焦点状态、ARIA 标签、可访问性 E2E 与截图证据。

...
```text
请先创建 `docs/agent/M3_17_INCIDENT_ALERT_EVIDENCE_PACK_CHECKLIST_UX_TASK.md`，再执行其中定义的 L5 超长任务。任务目标是在现有案件报告、SOC 时间线和告警研判能力之上，新增只读 evidence pack checklist UX，列出可下载报告、关联告警、时间线事件、脱敏状态和缺失项；只允许前端 evidence checklist UX、真实浏览器 E2E、截图和文档同步，不新增后端导出格式，不修改认证/授权/Guardrails/SSRF/DB schema/后端 API/npm 依赖/rate limit，不使用 localStorage/sessionStorage 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。通过新增 E2E、既有 incident/report/timeline/Dashboard 关键 E2E、后端全量、Guardrails、前端 typecheck/build 后，精确拆分 commit 并 push 到 `origin/main`，完成后输出最终报告。
```

### 下一条建议工单（M3-16 已固化后）

M3-16 已完成。下一条默认建议继续走产品体验路线：**M3-17 Incident / alert evidence pack checklist UX**，在现有案件报告、SOC 时间线和告警研判能力之上，补一个只读证据包清单视图，列出可下载报告、关联告警、时间线事件、脱敏状态和缺失项，不新增后端导出格式。

后续候选保留：

1. **Incident / alert evidence pack checklist UX**：在现有报告与时间线能力之上，补一个只读证据包清单视图，列出可下载报告、关联告警、时间线事件、脱敏状态和缺失项，不新增后端导出格式。

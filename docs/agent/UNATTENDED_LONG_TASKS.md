...
- `docs/agent/M3_12_DEMO_FLOW_E2E_STABILITY_TASK.md`：L5 级 M3-12 Demo Flow E2E 稳定性收口战役，覆盖 M3-11 遗留的 `test_demo_flow_e2e.py` 在多条浏览器 E2E 串跑时 Copilot no-key fallback 偶发 15s 超时问题；要求先用 `superpowers:systematic-debugging` 复现/采集证据，再新增 Copilot E2E 诊断 helper、条件等待、失败截图/JSON artifact、同一浏览器会话连续两次 Demo 分析稳定化测试、repeat 验证、六组关键 E2E 串跑、后端/Guardrails/前端质量门和通过后精确 push。**重要边界**：不放宽断言，不改认证/授权、Guardrails、SSRF、DB schema、后端 API、npm 依赖或 rate limit 常量，不提交 `.coverage` / env / DB / 密钥。**已交付**（2026-06-19）：新增 `server/tests/e2e_copilot_helpers.py`（`ARTIFACT_DIR=docs/runs/artifacts/m3-12-demo-flow-stability` + `_SENSITIVE_PATTERNS` sk-/AKIA/ghp_/xox- 正则脱敏 + `wait_for_copilot_fallback_message` 通过 `page.wait_for_function` 扫描 `[data-testid="copilot-message"][data-role="assistant"]` 含 `API Key`/`Base URL` + 默认 45s + `install_network_diagnostics` 仅记录 `/api/backend/copilot/stream` `/api/backend/alerts/demo` `/api/backend/health` `/api/auth/session` 的 method/path/status + `save_copilot_failure_artifacts` 失败时 full-page screenshot + sanitized JSON）；修改 `server/tests/test_demo_flow_e2e.py` 把 30×500ms 手写轮询替换为 `wait_for_copilot_fallback_message(page, timeout_ms=45000)` + 失败 artifact 落盘，保留严格断言；新增 `server/tests/test_demo_flow_stability_e2e.py` 同一 chromium context + page 连续两次 `trigger-demo-attack → analyze-current-alert → wait_for_copilot_fallback_message`，第二次 `attack-log-row` 数量 ≥ 2，整体跑完做 forbidden sentinel 扫描。**真实验证**：单条 Demo Flow E2E ×3 不重启 backend 全部 passed（10.38s/10.53s/10.51s）；Stability ×2 不重启 backend 全部 passed（10.28s/6.63s）；六组关键 E2E 连续 `pytest server/tests/test_auth_session_e2e.py test_demo_flow_e2e.py test_incident_report_e2e.py test_dashboard_route_sections_e2e.py test_dashboard_responsive_e2e.py test_demo_flow_stability_e2e.py --run-e2e` 7 passed in 58.27s；后端全量 `342 passed, 8 skipped, 17 warnings`；Guardrails 139 passed；前端 `npm run typecheck` 0 错误 + `npm run build` 通过（`/dashboard` 44 kB / First Load JS 191 kB 不变）。**根因排查**：本机环境 `curl https://api.openai.com` exit 28（5s 超时不可达）；fresh backend 时 `GuardrailEngine.check_input` 1.5s rail timeout 优先于 `OpenAIModerationClient` 5s ConnectTimeout 触发 → return None → 放行 Copilot；长时运行 backend 中 moderation httpx pool 退化后 < 1.5s 立即抛 exc → fail-closed 拦截 Copilot；本任务在 fresh backend 状态下行为正确，未动 Guardrails 代码。**未改**认证/授权、Guardrails、SSRF、DB schema、后端 API、npm 依赖、`REGISTER_RATE_LIMIT_*` / `COPILOT_RATE_LIMIT_*` 常量；未把 token 写进 `localStorage` / `sessionStorage` / DOM；未提交 `.coverage` / env / DB / 密钥。运行日志：`docs/runs/2026-06-19-m3-12-demo-flow-e2e-stability.md`。
- `docs/agent/M3_13_DASHBOARD_MOBILE_VISUAL_QA_TASK.md`：L5 级 M3-13 Dashboard 移动 viewport 视觉 QA 收口战役，基于 M3-11 `mobile-overview.png` / `mobile-incidents.png` 截图证据，覆盖移动 stats 卡片密度、section 间距、移动 nav active tab 可见性、整页横向溢出、forbidden sentinel、`N` 浮层 DOM 来源判断、新截图证据、M3-11/M3-12 E2E 回归、后端/Guardrails/前端质量门和通过后精确 push。**重要边界**：只做轻量 Tailwind/layout class 修复，不重做视觉设计，不改认证/授权、Guardrails、SSRF、DB schema、后端 API、npm 依赖或 rate limit 常量，不提交 `.coverage` / env / DB / 密钥。**已交付**（2026-06-19）：新增 `server/tests/test_dashboard_mobile_visual_e2e.py`，覆盖 390×844 / 430×932 下 `overview` / `incidents` 的 stats 密度、section 间距、移动 nav active tab、横向溢出、forbidden sentinel 与 `N` DOM 来源；轻量调整 `StatsCards.tsx`、`SystemStatusBar.tsx`、`DashboardBriefingSection.tsx`、`DashboardIncidentWorkspaceSection.tsx` 的 Tailwind/layout class；保存 `docs/runs/artifacts/m3-13-dashboard-mobile-visual/{mobile-390,mobile-430}-{overview,incidents}.png` 4 张截图；`N` 浮层 DOM candidates 为 0，判定为浏览器/外部 overlay。**真实验证**：移动视觉 E2E 1 passed；M3-11 responsive E2E 2 passed；M3-12 Demo stability E2E 1 passed；七组关键 E2E 8 passed；后端全量 `342 passed, 9 skipped, 17 warnings`；Guardrails `139 passed, 17 warnings`；前端 typecheck/build 通过（`/dashboard` 44.1 kB / First Load JS 191 kB）。运行日志：`docs/runs/2026-06-19-m3-13-dashboard-mobile-visual-qa.md`。
- `docs/agent/M3_14_INCIDENT_REPORT_PREVIEW_UX_TASK.md`：L5 级 M3-14 案件报告预览 UX 收口战役，基于 M3-07/M3-08 已交付的 Markdown 报告复制/下载能力，补齐 Dashboard 案件详情中的内联报告预览体验。任务覆盖新增 `IncidentReportPreview` 组件、`IncidentDetailPanel` 预览按钮与 loading/error/close 状态、报告文件名与 meta 展示、脱敏 markdown preview、复制/下载回归、桌面/移动截图证据、DOM/markdown forbidden sentinel、既有 incident report E2E 与移动视觉 E2E 回归、八组关键 E2E、后端/Guardrails/前端质量门和通过后精确 push。**重要边界**：不改认证/授权、Guardrails、SSRF、DB schema、后端 report API contract、npm 依赖或 rate limit 常量；不把报告 markdown 写入 `localStorage` / `sessionStorage`；不使用 `dangerouslySetInnerHTML`；不提交 `.coverage` / env / DB / 密钥。**已交付**（2026-06-20）：新增 `IncidentReportPreview.tsx` 内联预览组件，`IncidentDetailPanel.tsx` 接入 `预览报告` 按钮、loading/error/close/Escape 状态和截断 markdown preview；新增 `server/tests/test_incident_report_preview_e2e.py`，真实浏览器覆盖登录、Demo 告警、创建案件、预览报告、复制/下载回归、桌面/移动截图和 forbidden sentinel；保存 `docs/runs/artifacts/m3-14-incident-report-preview/incident-report-preview-{desktop,mobile}.png`。**真实验证**：新增 preview E2E 1 passed；既有 incident report E2E 1 passed；mobile visual E2E 1 passed；八组关键 E2E 9 passed；后端全量 `342 passed, 10 skipped, 17 warnings`；Guardrails `139 passed, 17 warnings`；前端 typecheck/build 通过（`/dashboard` 45.7 kB / First Load JS 193 kB）。运行日志：`docs/runs/2026-06-19-m3-14-incident-report-preview-ux.md`。
- `docs/agent/M3_15_SOC_TIMELINE_DRILLDOWN_FILTER_UX_TASK.md`：L5 级 M3-15 SOC 时间线筛选与详情展开 UX 收口战役，基于已交付的 `GET /logs/security-timeline` 和 `SecurityTimelinePanel`，把时间线从只读列表升级为可按 `全部 / Demo / Copilot / 护栏 / 系统` 筛选、可展开单条事件、可复制脱敏 SOC 摘要、可在桌面/移动截图中证明无泄漏的运营证据面板。任务覆盖新增真实浏览器 E2E、筛选状态、展开详情、Escape 收起、复制摘要降级、DOM/copy forbidden sentinel、后端 timeline 契约测试、Dashboard responsive 回归、九组关键 E2E、后端/Guardrails/前端质量门和通过后精确 push。**重要边界**：不改认证/授权、Guardrails、SSRF、DB schema、后端 timeline API contract、npm 依赖或 rate limit 常量；不使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`；不提交 `.coverage` / env / DB / 密钥。**已交付**（2026-06-20）：`SecurityTimelinePanel.tsx` 新增 `全部 / Demo / Copilot / 护栏 / 系统` 筛选、筛选计数、空筛选态、单条详情展开、Escape 收起和脱敏 SOC 摘要复制；详情只展示时间、来源、类别、状态、脱敏摘要和安全说明，不写 storage，不用 HTML 注入。新增 `server/tests/test_security_timeline_drilldown_e2e.py`，真实浏览器覆盖登录、Demo 事件进入时间线、筛选、展开详情、复制摘要、桌面/移动截图和 DOM/copy forbidden sentinel；保存 `docs/runs/artifacts/m3-15-soc-timeline-drilldown/security-timeline-{desktop,mobile}.png`。**真实验证**：新增 timeline drilldown E2E 1 passed；后端 timeline 契约 12 passed；Dashboard responsive E2E 2 passed；九组关键 E2E 10 passed；后端全量 `342 passed, 11 skipped, 17 warnings`；Guardrails `139 passed, 17 warnings`；前端 typecheck/build 通过（`/dashboard` 47.1 kB / First Load JS 194 kB）。运行日志：`docs/runs/2026-06-20-m3-15-soc-timeline-drilldown-filter-ux.md`。
- `docs/agent/M3_16_DASHBOARD_OPERATIONAL_RUNBOOK_HEALTH_CHECKLIST_UX_TASK.md`：L5 级 M3-16 Dashboard operational runbook / health checklist UX 收口战役，目标是在不修改认证/授权、Guardrails、SSRF、DB schema、后端 API、npm 依赖或 rate limit 的前提下，把后端健康、Next API 代理探针、登录会话、Demo readiness、E2E readiness 和 env security check 指引汇总成 owner 可自助排障的运维检查面板。**已交付**（2026-06-21）：新增 `OperationalRunbookPanel.tsx`，在 Dashboard 概览与 `WAF 管理` route 内展示六项检查、脱敏邮箱、五条人工执行命令和可复制安全诊断摘要；新增 `server/tests/test_dashboard_operational_runbook_e2e.py` 覆盖面板、复制、桌面/移动截图和 forbidden sentinel；测试 helper 增强显式稳定账号 env 复用以支撑长串 E2E，不改生产认证或后端 rate limit。真实验证：新增 runbook E2E 1 passed；Dashboard route + responsive 3 passed；关键 E2E 串跑 11 passed；后端全量 `344 passed, 12 skipped`；Guardrails 139 passed；前端 typecheck/build 通过。运行日志：`docs/runs/2026-06-21-m3-16-dashboard-operational-runbook-health-checklist-ux.md`。
- `docs/agent/M3_17_INCIDENT_ALERT_EVIDENCE_PACK_CHECKLIST_UX_TASK.md`：L5 级 M3-17 Incident / Alert evidence pack checklist UX 收口战役，目标是在现有案件报告、SOC 时间线和告警研判能力之上，新增只读证据包清单视图，列出报告可生成状态、关联告警、案件时间线、研判覆盖、脱敏/截断状态和缺失项。**已交付**（2026-06-21）：新增 `IncidentEvidencePackChecklist.tsx`，在案件详情关联告警与事件时间线之间展示六项只读检查；报告检查只通过既有 `onLoadReport(incidentId)` 读取 `filename/meta` 并丢弃 markdown；复制摘要只包含安全计数和状态字段，不包含 payload、原始 note、secret 或报告正文。新增 `server/tests/test_incident_evidence_pack_checklist_e2e.py` 覆盖 checklist、报告 meta、复制摘要、下载 markdown、桌面/移动截图和 DOM/clipboard/markdown forbidden sentinel。真实验证：新增 evidence pack E2E 1 passed；既有 incident report / preview / timeline / Dashboard responsive 5 passed；关键 E2E 串跑 12 passed；后端全量 `344 passed, 13 skipped`；Guardrails 139 passed；前端 typecheck/build 通过。运行日志：`docs/runs/2026-06-21-m3-17-incident-alert-evidence-pack-checklist-ux.md`。**重要边界**：未新增后端导出格式，未修改认证/授权、Guardrails、SSRF、DB schema、后端 incident/report API、npm 依赖或 rate limit；未调用 LLM；未使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`；未提交 `.coverage` / env / DB / 密钥。
- `docs/agent/M3_18_INCIDENT_CLOSURE_POST_INCIDENT_REVIEW_CHECKLIST_UX_TASK.md`：L5 级 M3-18 Incident closure / post-incident review checklist UX 收口战役，目标是在 M3-17 证据包清单之上，新增只读关闭前复盘清单，帮助 owner 判断案件是否适合进入 `resolved / false_positive`。**已交付**（2026-06-21）：新增 `IncidentClosureReviewChecklist.tsx`，在案件详情 Evidence Pack Checklist 之后、事件时间线之前展示八项关闭前复盘检查；报告检查只通过既有 `onLoadReport(incidentId)` 读取 `filename/meta` 并丢弃 markdown；复制摘要只包含安全计数、`closed_at` 是否适用、复盘备注是否存在、关闭建议和缺失项，不包含 payload、原始 note、secret 或报告正文。新增 `server/tests/test_incident_closure_review_checklist_e2e.py` 覆盖 checklist、报告 meta、复制摘要、保存 `contained` + 复盘备注、下载 markdown、桌面/移动截图和 DOM/clipboard/markdown forbidden sentinel。真实验证：新增 closure E2E 1 passed；M3-17 evidence pack + incident report / preview / timeline / Dashboard responsive 6 passed；关键 E2E 串跑 13 passed；后端 timeline 12 passed；后端全量 `344 passed, 14 skipped`；Guardrails 139 passed；前端 typecheck/build 通过。运行日志：`docs/runs/2026-06-21-m3-18-incident-closure-post-incident-review-checklist-ux.md`。**重要边界**：未修改认证/授权、Guardrails、SSRF、DB schema、后端 incident/report API、npm 依赖、rate limit 或 `closed_at` 语义；未自动关闭案件；未调用 LLM；未使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`；未提交 `.coverage` / env / DB / 密钥。
- `docs/agent/M3_19_CLOSED_INCIDENT_ARCHIVE_STATUS_FILTER_UX_TASK.md`：L5 级 M3-19 Closed Incident Archive / Status Filter UX 收口战役，目标是在案件列表中补状态筛选、关闭态归档视图和 `closed_at` 可见性。**已交付**（2026-06-21）：新增 `IncidentStatusFilterBar.tsx`，在案件列表上方提供 `全部 / 活跃 / 已开启 / 调查中 / 已遏制 / 已解决 / 误报 / 已关闭归档` 八类筛选；`IncidentSection.tsx` 复用既有 `GET /incidents?status=` 单状态接口，前端聚合 `active` 与 `closed`，并清理筛选后的 stale detail；`IncidentList.tsx` 新增归档模式，关闭态列表项展示结构化 `incident-closed-at`。新增 `server/tests/test_incident_status_filter_archive_e2e.py` 覆盖四类案件样本、八组筛选、关闭归档、M3-18 closure checklist 回归、桌面/移动截图和 DOM/storage forbidden sentinel。真实验证：新增 status filter E2E 1 passed；M3-18 closure + M3-17 evidence pack 2 passed；incident report + preview + Dashboard responsive 4 passed；后端 incident 契约 17 passed；关键 E2E 串跑失败项复跑 4 passed；后端全量 `344 passed, 15 skipped`；Guardrails 139 passed；前端 typecheck/build 通过。运行日志：`docs/runs/2026-06-21-m3-19-closed-incident-archive-status-filter-ux.md`。**重要边界**：未新增 DB schema，未改后端权限模型、认证/授权、Guardrails、SSRF、后端 incident/report API、npm 依赖、rate limit 或 `closed_at` 语义；未自动关闭案件，未调用 LLM，未使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`。
- `docs/agent/M3_20_INCIDENT_WORKBENCH_BULK_SELECTION_EXPORT_QUEUE_UX_TASK.md`：L5 级 M3-20 Incident Workbench Bulk Selection / Export Queue UX 收口战役，目标是在 M3-19 案件列表筛选与归档能力之上，新增列表多选、全选当前筛选、批量复制安全摘要、前端内存级导出队列提示、筛选切换清理 stale selection、桌面/移动截图和真实浏览器 E2E 证据。**已交付**（2026-06-22）：新增 `IncidentBulkActionBar.tsx`、`IncidentExportQueuePanel.tsx` 与 `incidentBulkActions.ts`；`IncidentList.tsx` 补 sibling checkbox，`IncidentSection.tsx` 用 React state 管理 selection/export queue 并在筛选或列表刷新时清理 stale selection；批量摘要只包含 incident id、状态、严重度、告警数、更新时间、`closed_at` 是否存在和 title length，不包含 title 正文、summary、payload、note、报告 markdown、secret 或 stack trace。新增 `server/tests/test_incident_bulk_selection_export_queue_e2e.py` 覆盖多选、全选、清空、复制摘要、队列、筛选清理、详情打开回归、刷新后不从 storage 恢复、桌面/移动截图和 DOM/clipboard/storage forbidden sentinel。真实验证：新增 bulk selection E2E 1 passed；相邻案件 UX + Dashboard responsive 7 passed；关键 E2E 串跑 15 passed；后端 incident 契约 17 passed；后端全量 `344 passed, 16 skipped`；Guardrails 139 passed；前端 typecheck/build 通过。运行日志：`docs/runs/2026-06-22-m3-20-incident-workbench-bulk-selection-export-queue-ux.md`。**重要边界**：未新增后端导出格式，未改后端权限模型、认证/授权、Guardrails、SSRF、后端 incident/report API、DB schema、npm 依赖或 rate limit；未自动关闭/删除/批量修改案件，未调用 LLM，未使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`。
- `docs/agent/M3_21_INCIDENT_WORKSPACE_KEYBOARD_NAVIGATION_ACCESSIBILITY_QA_UX_TASK.md`：L5 级 M3-21 Incident Workspace Keyboard Navigation / Accessibility QA UX 收口战役，目标是在 M3-17 到 M3-20 的案件工作台能力之上，补键盘导航、焦点状态、ARIA 标签、报告预览焦点恢复、Evidence / Closure checklist 可访问性、桌面/移动截图和真实浏览器 E2E 证据。**已交付**（2026-06-22）：`IncidentSection.tsx` 补工作台 region；`IncidentList.tsx` 将 checkbox accessible name 收敛到 incident id、状态、严重度、告警数，并补 `aria-current` / `aria-selected`；`IncidentDetailPanel.tsx` 支持 status / severity radiogroup 方向键、Home/End 导航和报告预览焦点恢复；Evidence / Closure checklist、导出队列和批量操作补 list/listitem 语义与 focus-visible。新增 `server/tests/test_incident_workspace_accessibility_e2e.py` 覆盖键盘筛选、Space 多选、Tab/Enter 打开详情、radiogroup 键盘导航、报告预览焦点恢复、Evidence/Closure 刷新复制、批量复制/队列清空、accessible name audit、重复 id audit、storage audit、DOM/clipboard forbidden sentinel 和桌面/移动截图。真实验证：新增 keyboard/a11y E2E 1 passed；相邻案件 UX 7 个真实浏览器测试均有通过证据；关键 E2E 15 个文件 / 16 个真实浏览器测试均有通过证据；后端 incident/report 契约 31 passed；后端全量 `344 passed, 17 skipped`；Guardrails 139 passed；前端 typecheck/build 通过。运行日志：`docs/runs/2026-06-22-m3-21-incident-workspace-keyboard-navigation-accessibility-qa-ux.md`。**重要边界**：只改前端可访问性/键盘导航 UX、E2E、截图和文档同步；未新增后端能力，未改认证/授权、Guardrails、SSRF、后端 incident/report API、DB schema、npm 依赖或 rate limit，未自动关闭/删除/批量修改案件，未调用 LLM，未使用 `localStorage` / `sessionStorage` 或 `dangerouslySetInnerHTML`。
...
```text
请创建并执行 `docs/agent/M3_22_INCIDENT_WORKSPACE_GUIDED_REVIEW_SESSION_OPERATOR_HANDOFF_UX_TASK.md` 中定义的 L5 超长任务：新增 Incident Workspace Guided Review Session / Operator Handoff UX。先完成任务文档、必读上下文、运行日志、RED→GREEN→IMPROVE、真实浏览器 E2E、截图、文档同步、验证矩阵、精确 commit 和 push；只允许前端只读 review session / operator handoff 摘要 UX、E2E、截图和文档同步，不新增后端 API，不改认证/授权、Guardrails、SSRF、后端 incident/report API、DB schema、npm 依赖或 rate limit，不自动关闭/删除/批量修改案件，不调用 LLM，不使用 localStorage/sessionStorage 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。完成后输出最终报告。
```

### 下一条建议工单（M3-21 已交付后）

M3-21 Incident Workspace Keyboard Navigation / Accessibility QA UX 已交付。下一条默认建议继续走产品体验路线：**M3-22 Incident Workspace Guided Review Session / Operator Handoff UX**，在不新增后端 API 和不调用 LLM 的前提下，把当前筛选、已选案件、Evidence Pack、Closure Review、报告可用性和队列状态组织成前端只读 review session 面板，并提供安全 handoff 摘要复制与 E2E 证据。

后续候选保留：

1. **Incident Workspace Guided Review Session / Operator Handoff UX**：把当前筛选、已选案件、Evidence Pack、Closure Review、报告可用性和队列状态组织成前端只读 review session 面板，并提供安全 handoff 摘要复制与 E2E 证据。
2. **Guardrails moderation httpx pool 健康监控（需 owner 单独授权）**：M3-12 根因排查暴露长时运行 backend 中 `OpenAIModerationClient._shared_client` 偶发退化导致 moderation 在 < 1.5s rail timeout 内立即抛 exc → fail-closed 拦截 Copilot；fresh backend 行为正确。该工单会触碰 `server/security/llm_guardrails/**`，必须 owner 单独授权。

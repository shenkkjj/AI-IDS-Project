...
```text
请执行 `docs/agent/NEXT_02_E2E_AND_SSRF_QUALITY_GATE_HARDENING_TASK.md` 中定义的 L5 超长任务。先完整阅读该文件和其中列出的必读上下文，创建运行日志，按 TDD 先复现 `test_demo_flow_e2e.py --run-e2e` 的旧登录 / 跳转等待问题和 `test_ssrf.py` 对真实 DNS 的依赖，再最小修复 Demo Flow E2E：复用 NEXT-01 已验证的后端 API register + NextAuth callback cookie seeding + `/dashboard` URL polling / explicit goto，不要再用 `expect_navigation` 等待 App Router client-side route；SSRF 只改测试 monkeypatch，严禁放宽生产 `_is_ssrf_safe` / `_is_url_pointing_to_internal`。不要问我小问题，不要跳过/xfail 测试，不要删除 Copilot fallback、triage、DOM forbidden sentinel 断言，不要改生产认证、Guardrails、数据库 schema、部署配置，不要使用 git add .，不要提交 `.coverage`、真实 env、数据库或密钥。满足 Demo Flow E2E、Auth E2E、Incident Report E2E、SSRF、后端全量、Guardrails、前端 typecheck/build 质量门后，可以按文档精确 commit 并 push 到 `origin/main`，完成后按任务文档输出最终报告。
```

### 下一条建议工单（NEXT-02 已交付后）

**L5 / M3-09：Dashboard incident state 单一事实源收口**。NEXT-02 已让 Demo Flow / Auth / Incident Report 三条浏览器 E2E 真实通过，并固化 SSRF 测试 DNS 隔离；目前残留的产品级 bug 是 `IncidentSection` 与 `dashboard-client.tsx` 各自独立 `useIncidents()` 实例不共享 state（M3-04 / NEXT-01 遗留），E2E 通过点击列表项规避。下一条工单建议把 `incidentsCtx` 提到 `dashboard-client.tsx` 父组件统一持有，或让 `IncidentSection` 接受父组件传入的 selected incident state，避免双 hook 之间的同步 race。同时建议给 E2E 注册路径加 IP 白名单或 fixture-level backoff，避免 `REGISTER_RATE_LIMIT_MAX=5/小时` 在连续跑多条 E2E 时触发 429。完成后目标：单一 incident state 源、E2E 不再依赖 dev server 重启、双 hook 风险消失。

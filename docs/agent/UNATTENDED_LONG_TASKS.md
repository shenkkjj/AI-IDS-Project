...
```text
请执行 `docs/agent/M3_08_INCIDENT_REPORT_BROWSER_E2E_AND_AGENT_DOCS_CATCHUP_TASK.md` 中定义的 L5 超长任务。先完整阅读该文件和其中列出的必读上下文，创建运行日志，优先新增或扩展可选 Playwright E2E，覆盖注册/登录、触发 Demo 告警、创建案件、打开案件详情、下载 Markdown 案件报告、验证报告结构与脱敏 sentinel、点击复制报告、扫描页面 DOM 无泄漏；同时把遗留的 M3-07/M3-08 agent 任务文档归档入库并更新无人值守索引。不要问我小问题，不要新增 API，不要新增数据库迁移，不要改认证/授权/Guardrails，不要引入 PDF/DOCX/外部渲染或 LLM 报告生成，不要使用 git add .，不要提交 `.coverage`、`.claude/settings.local.json`、真实 env、数据库或密钥。满足质量门后可以按文档精确 commit 并 push 到 `origin/main`，完成后按任务文档输出最终报告。
```

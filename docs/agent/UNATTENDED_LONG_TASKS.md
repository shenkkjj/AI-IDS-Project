...
```text
请执行 `docs/agent/M3_09_INCIDENT_STATE_AND_E2E_RESILIENCE_TASK.md` 中定义的 L5 超长任务。先完整阅读该文件和其中列出的必读上下文，创建运行日志，按 TDD 先复现 Incident Report E2E 当前必须点击 `incident-list-item` 才能出现 `incident-detail-panel` 的双 `useIncidents()` 状态 race，再把 `useIncidents()` 提升到 `dashboard-client.tsx` 父层并作为 controller 传给 `IncidentSection`，让从告警创建案件后案件列表、selected incident、detail、复制/下载报告都共享单一事实源；同时新增复用的 `server/tests/e2e_helpers.py`，统一 Auth / Demo Flow / Incident Report 三条 E2E 的 register + NextAuth callback cookie helper，处理 409/已存在和稳定测试账号路径，但严禁放宽生产 `REGISTER_RATE_LIMIT_MAX`、后端 auth/token/cookie 语义、Guardrails、SSRF 或数据库 schema。不要问我小问题，不要跳过/xfail 浏览器 E2E，不要把 token 写进 localStorage/sessionStorage/DOM，不要使用 git add .，不要提交 `.coverage`、真实 env、数据库或密钥。满足 helper 单测、三条单独 E2E、三条连续 E2E、后端全量、Guardrails、前端 typecheck/build 质量门后，可以按文档精确 commit 并 push 到 `origin/main`，完成后按任务文档输出最终报告。
```

### 下一条建议工单（NEXT-02 已交付后）

**L5 / M3-09：Dashboard incident state 单一事实源收口**。NEXT-02 已让 Demo Flow / Auth / Incident Report 三条浏览器 E2E 真实通过，并固化 SSRF 测试 DNS 隔离；当前残留的产品级 bug 是 `IncidentSection` 与 `dashboard-client.tsx` 各自独立 `useIncidents()` 实例不共享 state，E2E 仍通过点击列表项规避。本工单已固化为 `docs/agent/M3_09_INCIDENT_STATE_AND_E2E_RESILIENCE_TASK.md`，目标是让案件 state 单一事实源、Incident Report E2E 不再依赖列表点击、连续 E2E 不再依赖 dev server 重启，同时保持生产注册限流与认证安全边界不降级。

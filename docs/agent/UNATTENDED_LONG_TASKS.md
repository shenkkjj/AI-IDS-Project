...
```text
请执行 `docs/agent/M3_19_CLOSED_INCIDENT_ARCHIVE_STATUS_FILTER_UX_TASK.md` 中定义的 L5 超长任务：新增 Closed Incident Archive / Status Filter UX。严格按文档创建运行日志、RED→GREEN→IMPROVE、真实浏览器 E2E、截图、文档同步、验证矩阵、精确 commit 和 push 执行；只允许前端案件列表筛选/关闭态归档 UX、E2E、截图和文档同步，不新增 DB schema，不改后端权限模型、认证/授权、Guardrails、SSRF、后端 incident/report API、npm 依赖或 rate limit，不自动关闭案件，不调用 LLM，不使用 localStorage/sessionStorage 或 `dangerouslySetInnerHTML`，不要提交 `.coverage`、真实 env、数据库或密钥。完成后输出最终报告。
```

### 下一条建议工单（M3-19 已交付后）

M3-19 Closed Incident Archive / Status Filter UX 已交付。默认建议继续走产品体验路线：**M3-20 Incident Workbench Bulk Selection / Export Queue UX**，在不新增后端导出格式和不改权限模型的前提下，为案件列表增加多选、批量复制安全摘要、导出队列提示和 E2E 证据。

后续候选保留：

1. **Incident Workbench Bulk Selection / Export Queue UX**：在不新增后端导出格式和不改权限模型的前提下，为案件列表增加多选、批量复制安全摘要、导出队列提示和 E2E 证据。

...
```text
请执行 `docs/agent/NEXT_01_AUTH_SESSION_LOADING_E2E_RECOVERY_TASK.md` 中定义的 L5 超长任务。先完整阅读该文件和其中列出的必读上下文，创建运行日志，按 TDD 先新增最小 auth E2E 复现 Dashboard 登录后 `useSession` 永 loading / `SYSTEM · LOADING` 不消失的问题，再优先用服务端 `auth()` 保护 `/dashboard` 并渲染 `DashboardClient`，必要时再给 `SessionProvider` 注入服务端初始 session 或最小升级 next-auth/Next.js；不要问我小问题，不要绕过认证，不要把 token 放进 localStorage/sessionStorage，不要暴露 backend access token，不要改后端 auth API/schema/Guardrails，不要使用 git add .，不要提交 `.coverage`、真实 env、数据库或密钥。满足 `test_auth_session_e2e.py --run-e2e`、M3-08 `test_incident_report_e2e.py --run-e2e`、后端全量、Guardrails、前端 typecheck/build 质量门后，可以按文档精确 commit 并 push 到 `origin/main`，完成后按任务文档输出最终报告。
```

### 下一条建议工单（已交付 M3-08 后）

**L3 / NEXT-01：解决 next-auth 5 beta + Next.js 15 dev mode `useSession` 永 loading 阻塞（dashboard 不可见）**。M3-08 E2E 阻塞的根因；M3-04 / M3-07 报告导出前端 UI 实际未被真实浏览器覆盖，属于产品化阻塞。本工单授权修改 `web-next/lib/auth.ts` / `web-next/app/providers.tsx` / `web-next/package.json`（next-auth / Next.js 升级）以解决 React hydration 与 SessionProvider 兼容性问题。完成后重启 E2E 走 M3-08 完整流程，目标：`--run-e2e` 真实通过；后端契约 / 全量 / Guardrails / 前端 typecheck / build / 浏览器级 E2E 全绿。

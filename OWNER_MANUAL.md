...
- 生成安全密钥（APP_SECRET、AUTH_SECRET）
- 生成数据库密码（POSTGRES_PASSWORD、REDIS_PASSWORD）
- 构建并启动所有 Docker 容器
...
# ---------- 安全核心 ----------
APP_SECRET=your-secret-at-least-32-chars
...
POSTGRES_PASSWORD=your-strong-password    # Docker部署必填
REDIS_PASSWORD=your-strong-password       # Docker部署必填
...
> ⚠️ **重要**：Docker Compose 部署时，以下密钥必须在 `.env` 中配置：

| 密钥 | 说明 | 自动生成 |
|------|------|----------|
| `APP_SECRET` | JWT 签名密钥 | ✅ deploy.ps1 |
| `AUTH_SECRET` | NextAuth 加密密钥 | ✅ deploy.ps1 |
| `POSTGRES_PASSWORD` | PostgreSQL 数据库密码 | ✅ deploy.ps1 |
| `REDIS_PASSWORD` | Redis 缓存密码 | ✅ deploy.ps1 |
...
> ⚠️ **Docker 部署必读**：以下密钥在 Docker Compose 部署时必须通过环境变量传入，禁止使用默认值。

| 密钥 | 位置 | 要求 | 自动生成 |
|------|------|------|----------|
| `APP_SECRET` | `.env` | 至少 32 字符 | ✅ deploy.ps1 |
| `AUTH_SECRET` | `.env` | 至少 32 字符 | ✅ deploy.ps1 |
| `POSTGRES_PASSWORD` | `.env` | 至少 16 字符 | ✅ deploy.ps1 |
| `REDIS_PASSWORD` | `.env` | 至少 16 字符 | ✅ deploy.ps1 |
| `APP_FERNET_KEY` | `.env` | 留空则自动生成 | ✅ |
| `LLM_ADMIN_TOKEN` | `.env` | 访问 `/llm/config` 的管理令牌 | ❌ 需手动配置 |

> 生产环境必须使用 `openssl rand -base64 48` 生成真正的随机密钥。
...
*文档版本：v6.0 | 更新日期：2026-05-08 | 包含 Docker 安全配置和 Windows 一键部署*

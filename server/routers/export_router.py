...
def _get_db():
    from server.core.database import get_db as _gdb
    return next(_gdb())


async def _get_user_id(request: Request, db):
...
async def export_logs_csv(
    ...
    try:
        user_id = await _get_user_id(request, db)
        result = db.execute(
            text("SELECT id, level, action, detail, ip_address, created_at FROM logs WHERE user_id = :uid OR user_id IS NULL ORDER BY created_at DESC LIMIT :lim"),

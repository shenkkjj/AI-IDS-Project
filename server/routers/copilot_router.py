...
async def post_threat_confirm(
    ...
    alert = await find_alert_by_id(data.alert_id, user_id=user.id)
    if alert is None:
        raise HTTPException(status_code=404, detail="alert_id not found")

    await append_new_threat_csv(alert, data.label)
    create_log(db, user_id=user.id, level="info", action="threat_confirm",
               detail=f"{data.alert_id}:{_sanitize_for_log(data.label)}")
    return {
        "status": "ok",
